...
import kotlinx.android.synthetic.main.ware_other_transfer_fragment3.*
...
import ykk.xc.com.wms.warehouse.adapter.Ware_Other_Transfer_Fragment3_Adapter
 ...
 * 作者：ykk
 */
class Ware_Other_Transfer_Fragment3 : BaseFragment() {
    ...
    private var mAdapter: Ware_Other_Transfer_Fragment3_Adapter? = null
    ...
    private class MyHandler(activity: Ware_Other_Transfer_Fragment3) : Handler() {
        private val mActivity: WeakReference<Ware_Other_Transfer_Fragment3>
...
//                            m.parent!!.fragment1.reset() // 重置
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_other_transfer_fragment3, container, false)
    }
    ...
    override fun initView() {
        ...
        mAdapter = Ware_Other_Transfer_Fragment3_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : Ware_Other_Transfer_Fragment3_Adapter.MyCallBack {
...
//                    if(it.fqty > 0 && it.stockId2_wms == 0) {
                    ...
                    // 调出仓库判断
                    if(it.stock2.useStockArea.equals("Y") && it.stockAreaId2_wms == 0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，调出仓库启用了库区，请选择库区！")
                        return
                    }
                    if(it.stock2.useStorageRack.equals("Y") && it.storageRackId2_wms == 0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，调出仓库启用了货架，请选择货架！")
                        return
                    }
                    if(it.stock2.fisStockMgr == 1 && it.stockPosId2_wms == 0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，调出仓库启用了库位，请选择库位！")
                        return
                    }
                    // 调入仓库判断
                    if(it.stock.useStockArea.equals("Y") && it.stockAreaId_wms == 0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，调入仓库启用了库区，请选择库区！")
                        return
                    }
                    if(it.stock.useStorageRack.equals("Y") && it.storageRackId_wms == 0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，调入仓库启用了货架，请选择货架！")
                        return
                    }
                    if(it.stock.fisStockMgr == 1 && it.stockPosId_wms == 0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，调入仓库启用了库位，请选择库位！")
                        return
                    }
//                    if(it.fqty == 0.0) {
