...
import kotlinx.android.synthetic.main.sal_outstock_red_fragment3.*
...
import ykk.xc.com.wms.sales.adapter.Sal_OutStock_RED_Fragment3_Adapter
...
class Sal_OutStock_RED_Fragment3 : BaseFragment() {
    ...
    private var mAdapter: Sal_OutStock_RED_Fragment3_Adapter? = null
    ...
    private class MyHandler(activity: Sal_OutStock_RED_Fragment3) : Handler() {
        private val mActivity: WeakReference<Sal_OutStock_RED_Fragment3>
...
//                        m.parent!!.fragment1.reset() // 重置
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.sal_outstock_red_fragment3, container, false)
    }
    ...
    override fun initView() {
        ...
        mAdapter = Sal_OutStock_RED_Fragment3_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : Sal_OutStock_RED_Fragment3_Adapter.MyCallBack {
            ...
            R.id.btn_save -> { // 上传
                ...
                checkDatas.forEachIndexed { index, it ->
                    ...
                    if(it.fqty > it.fsourceQty) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，退货不能大于源单数！")
                        return
                    }
                }
...
//            }
     ...
     * 保存生成销售装箱任务单
     */
    private fun run_upload() {
        var strJson = ""
        var strIcstockBill = ""
        var strIcstockBillEntry = ""
        var missionType = ""
        showLoadDialog("加载中...", false)
        var mUrl:String? = null
        // 发货方式( 发货运:990664，送货:990665 )
        if(parent!!.fragment1.icStockBill.deliverWay == 990664) {
            mUrl = getURL("stockBill_WMS/saveToMissionBill")
            strIcstockBill = JsonUtil.objectToString(parent!!.fragment1.icStockBill)
            strIcstockBillEntry = JsonUtil.objectToString(checkDatas)
            missionType = "54"
        } else {
            mUrl = getURL("stockBill_WMS/uploadToK3")
            val list = ArrayList<ICStockBill>()
            list.add(parent!!.fragment1.icStockBill)
            strJson = JsonUtil.objectToString(list)
        }
        val formBody = FormBody.Builder()
                .add("strJson", strJson)
