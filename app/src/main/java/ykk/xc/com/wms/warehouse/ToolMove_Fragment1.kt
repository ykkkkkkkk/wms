...
import kotlinx.android.synthetic.main.ware_tool_move_fragment1.*
import okhttp3.*
 ...
 * 作者：ykk
 */
class ToolMove_Fragment1 : BaseFragment() {
    ...
    private class MyHandler(activity: ToolMove_Fragment1) : Handler() {
        private val mActivity: WeakReference<ToolMove_Fragment1>
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_tool_move_fragment1, container, false)
    }
    ...
    override fun initData() {
        ...
        record.sourceType = '1' // 来源数据类型	1：容器，2：工装模具
        record.useType = 'L' // 使用类型	L：领用，H：归还
        record.useMan = user!!.username
    }
    ...
    @OnClick( R.id.btn_positionScan, R.id.btn_positionSel, R.id.btn_positionScan2, R.id.btn_positionSel2,
    ...
    fun onViewClicked(view: View) {
        when (view.id) {
            ...
            R.id.btn_save -> { // 保存
                if(record.returnContainerId == 0) {
                    Comm.showWarnDialog(mContext,"请扫描容器条码或选择！")
                    return
                }
    ...
    override fun setListener() {
        ...
        et_positionCode!!.setOnLongClickListener {
            smqType = 1
            showInputDialog("输入容器条码", getValues(et_positionCode), "none", WRITE_CODE)
            true
        }
...
//        parent!!.isChange = false
     ...
     * 得到扫码或选择数据
     */
    private fun getContainer(m : Container) {
        tv_stockName.text = "仓库："
        tv_stockAreaName.text = "库区："
        tv_storageRackName.text = "货架："
        tv_stockPosName.text = "库位："
        ...
        if(m.stockPosition != null ) {
            tv_stockPosName.text = Html.fromHtml("库位：<font color='#6a5acd'>"+m.stockPosition.stockPositionName+"</font>")
            tv_stockPosName.visibility = View.VISIBLE
            record.returnStockPosId = m.stockPosition.id
        }
        tv_containerName.text = Html.fromHtml("容器：<font color='#6a5acd'>"+m.fnumber+"</font>")
