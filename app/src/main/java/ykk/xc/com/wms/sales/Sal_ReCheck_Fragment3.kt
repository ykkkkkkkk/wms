...
import kotlinx.android.synthetic.main.sal_recheck_fragment3.*
...
import ykk.xc.com.wms.sales.adapter.Sal_ReCheck_Fragment3_Adapter
...
class Sal_ReCheck_Fragment3 : BaseFragment() {
    ...
    private var mAdapter: Sal_ReCheck_Fragment3_Adapter? = null
    ...
    private class MyHandler(activity: Sal_ReCheck_Fragment3) : Handler() {
        private val mActivity: WeakReference<Sal_ReCheck_Fragment3>
...
//                        m.parent!!.fragment1.reset() // 重置
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.sal_recheck_fragment3, container, false)
    }
    ...
    override fun initView() {
        ...
        mAdapter = Sal_ReCheck_Fragment3_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : Sal_ReCheck_Fragment3_Adapter.MyCallBack {
        ...
        when (view.id) {
            R.id.btn_save -> { // 上传
                ...
                /*checkDatas.forEachIndexed { index, it ->
                    ...
                    if(it.fqty == 0.0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，请扫码或输入（复核数）！")
                        return
                    }
                    if(it.fqty > it.fsourceQty) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，复核数不能大于合格数！")
