...
class Sal_PickGoods_Fragment2 : BaseFragment() {
    ...
    private class MyHandler(activity: Sal_PickGoods_Fragment2) : Handler() {
        private val mActivity: WeakReference<Sal_PickGoods_Fragment2>
...
//                                if(m.getValues(m.tv_mtlName).length > 0) {
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.sal_pickgoods_fragment2, container, false)
    }
...
//        }
        ...
        if (icStockBillEntry.fqty == 0.0) {
            Comm.showWarnDialog(mContext, "请输入拣货数！")
            return false
        }
        if (icStockBillEntry.fqty > icStockBillEntry.fsourceQty) {
            Comm.showWarnDialog(mContext, "拣货数不能大于源单数！")
            return false
        }
