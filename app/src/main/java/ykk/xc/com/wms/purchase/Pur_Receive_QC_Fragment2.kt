...
import kotlinx.android.synthetic.main.pur_receive_qc_fragment1.tv_suppSel
import kotlinx.android.synthetic.main.pur_receive_qc_fragment2.*
import okhttp3.*
 ...
 * 作者：ykk
 */
class Pur_Receive_QC_Fragment2 : BaseFragment() {

    companion object {
        private val SEL_POSITION = 11
        private val SEL_MTL = 12
        private val SEL_UNIT = 13
        ...
        private val RESULT_POSITION = 11
    }
    ...
    private class MyHandler(activity: Pur_Receive_QC_Fragment2) : Handler() {
        private val mActivity: WeakReference<Pur_Receive_QC_Fragment2>
...
//                                }
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.pur_receive_qc_fragment2, container, false)
    }
    ...
    @OnClick(R.id.btn_scan, R.id.btn_mtlSel, R.id.btn_positionScan, R.id.btn_positionSel,
             R.id.tv_qcPassQty, R.id.tv_weight, R.id.tv_batchNo, R.id.tv_unitSel, R.id.tv_remark, R.id.btn_save, R.id.btn_clone,
             R.id.tv_weightUnitType, R.id.tv_connBlueTooth2, R.id.tv_positionName, R.id.tv_icItemName)
...
//                showInputDialog("单价", icStockBillEntry.fprice.toString(), "0.0", RESULT_PRICE)
            ...
            R.id.tv_qcPassQty -> { // 合格数量
                showInputDialog("合格数量", icStockBillEntry.qcPassQty.toString(), "0.0", RESULT_NUM)
            }
            R.id.tv_weight -> { // 称重数量
...
//            return false;
//        }
//        if(icStockBillEntry.icItem.batchManager.equals("Y")) {
//            Comm.showWarnDialog(mContext, "请输入批次！")
//            return false
//        }
        if (icStockBillEntry.qcPassQty == 0.0) {
            Comm.showWarnDialog(mContext, "请输入合格数量！")
            return false;
        }
...
//                        icStockBillEntry.fqty = roundQty
                        icStockBillEntry.qcPassQty = roundQty
                        tv_qcPassQty.text = df.format(roundQty)
                    },150)
                }
            }
        })
    ...
    private fun reset(flag : Int) {
        ...
        setEnables(tv_qcPassQty, R.drawable.back_style_blue, true)
...
//        icStockBillEntry.fdcSPId = 0
        ...
        icStockBillEntry.qcPassQty = 0.0
...
//        tv_batchNo.text = icEntry.fbatchNo
        ...
        if(icEntry.icItem.batchManager.equals("Y") || icEntry.icItem.snManager.equals("Y")) {
            setEnables(tv_qcPassQty, R.drawable.back_style_gray3, false)
        } else {
            setEnables(tv_qcPassQty, R.drawable.back_style_blue, true)
        }
//        tv_qcPassQty.text = df.format(icEntry.qcPassQty)
        ...
        tv_inStockQty.text = df.format(icEntry.fqty)
        tv_unitSel.text = icEntry.unitName
        /*if(Comm.isNULLS(icEntry.fkfDate).length == 0) {
...
//        tv_sumMoney.text = df.format(mul)
        ...
        if(icEntry.icstockBillEntry_Barcodes.size > 0 && icEntry.icItem.batchManager.equals("N") && icEntry.icItem.snManager.equals("N")) {
            showBatch_Qty(null, icEntry.qcPassQty)
        } else {
            // 显示多批次
//        showBatch_Qty(icEntry.icstockBillEntry_Barcodes, icEntry.fqty)
            showBatch_Qty(smICStockBillEntry_Barcodes, icEntry.qcPassQty)
        }
        mHandler.postDelayed(Runnable {
            ...
            RESULT_NUM -> { // 数量	返回
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    if (bundle != null) {
                        ...
                        tv_qcPassQty.text = df.format(num)
//                        icStockBillEntry.fqty = num
                        icStockBillEntry.qcPassQty = num
...
//                        }
    ...
    fun showBatch_Qty(list : List<ICStockBillEntry_Barcode>?, fqty : Double) {
        if(list != null && list.size > 0) {
            ...
            tv_qcPassQty.text = df.format(sumQty)

            icStockBillEntry.qcPassQty = sumQty
            icStockBillEntry.icstockBillEntry_Barcodes.clear()
            icStockBillEntry.icstockBillEntry_Barcodes.addAll(list)
        } else {
            icStockBillEntry.fbatchNo = ""
            icStockBillEntry.qcPassQty = fqty
            tv_batchNo.text = ""
            tv_qcPassQty.text = if(fqty > 0) df.format(fqty) else ""
        }
    }
...
//            mUrl = getURL("stockBill_WMS/saveEntryList")
            mUrl = getURL("stockBill_WMS/addEntryBySelect")
